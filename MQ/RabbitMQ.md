...
### 备份交换机  
为正常交换机创建对应的备份交换机，用于接收消息生产者生产的没有绑定对应队列的消息，当交换机接收到一条不可路由的消息，会将该消息转发到备份交换机，由备份交换机来进行转发处理，一般备份交换机的类型为fanout类型交换机，使用消费者监听订阅了交换机的消息队列就可以达到处理不可路由消息的需求。   
### 优先队列  
定义一个优先队列，对生产者发送的每条消息定义优先级，消费者需要等待消息都到队列进行优先级排序后，再消费。  
### 惰性队列  
惰性队列会将消息尽可能存在磁盘中，只有当消费者消费到该消息时，才会将消息从磁盘读到内存中，这样单个队列能存储更多的消息。默认的队列是尽可能将消息存在内存中，这样当消费者消费时，可以更快将消息发送给消费者，即使是持久化队列，在写入磁盘时，也会将消息加载到内存里备份。当消息队列需要释放内存时，将内存中的数据换页到磁盘中，这个过程较长，且会阻塞队列的操作，无法接收新的消息。
## 实战应用场景  
### 消息的顺序消费  
...
解决方法：消费的幂等性
1、对消息的内容进行唯一性约束（比如入库时，校验消息的唯一性属性）  
2、使用redis的setnx命令，天然具有幂等性。